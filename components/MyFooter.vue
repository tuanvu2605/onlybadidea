<template>
  <footer class="footer">
    <div class="wrapper">
      <div class="info-bottom">
        <img src="~/assets/images/logo-bottom.png" alt="" title="" />
        <p>© 2022 Gamma Play. All Rights Reserved. <a href="http://gammaplay.com/privacy_policy.php"><br>
          Privacy Policy</a> | <a href="http://gammaplay.com/tos.php">Terms of Service</a></p>
      </div><!-- info-bottom -->

      <div class="footer-middle-wrapper">
        <div class="footer-middle">
          <h4 class="blue">CONTACT / SUPPORT</h4>
          <p>Contact us if you need any help or would like to publish your new app or game with us. <br>
            Email &#103;&#097;&#109;&#109;&#097;&#112;&#108;&#097;&#121;&#046;&#099;&#111;&#109;&#064;&#103;&#109;&#097;&#105;&#108;&#046;&#099;&#111;&#109;</p>
          <h4 class="red">YOUTUBERS</h4>
          <p>You have our permission to use game footage and post what you have on YouTube.</p>
        </div><!-- footer-middle -->
      </div><!-- footer-middle-wrapper -->

      <div class="social">
        <h4>SOCIAL MEDIA</h4>
        <div class="twitter">
          <a href="https://twitter.com/GammaPlay" class="twitter-follow-button" data-show-count="false" data-size="large">Follow @GammaPlay</a>
        </div><!-- twitter -->
        <div class="facebook">
          <div class="fb-like" data-href="https://www.facebook.com/gammaplay/" data-width="240"  data-layout="standard" data-action="like" data-size="large" data-show-faces="false" data-share="true" data-colorscheme="light">
          </div><!-- facebook -->
        </div><!-- social -->
      </div><!-- wrapper -->
    </div>
  </footer><!-- footer -->
</template>

<script>
export default {
  name: 'MyFooter',
  data: () => ({

  }),
  methods: {

  }
}
</script>
